...
class ControllerLogin extends GetxController {
  ...
  final RepositoryLogin _repositoryLogin = Get.put<RepositoryLogin>(RepositoryLoginImp());
  ...
  Future<void> logout() async {
    try {
      ...
      Get.snackbar('Ok', 'Se ha cerrado la sesión', snackPosition: SnackPosition.BOTTOM, margin: const EdgeInsets.all(20), maxWidth: 500);
    } on FirebaseAuthException catch (e) {
      Get.snackbar('Error', 'Ha ocurrido un error al cerrar sesión', snackPosition: SnackPosition.BOTTOM, margin: const EdgeInsets.all(20), maxWidth: 500);
    }
  }
  ...
  Future<void> signInWithGoogle(BuildContext context) async {
    try {
      ...
      if (googleUser != null) {
        ...
        _repositoryLogin.registerUserWithGoogle(userLogin).then((value) => Get.back());
      }
    } catch (e) {
      print(e);
  ...
  Future<bool> getLoginUser(BuildContext context)async {
    ...
    if (formKey.currentState!.validate() && isValidEmail) {
      if (controller.validPassword(passwordL.text.trim())) {
        ...
        return await accessLoginUser(userLogin).then((value)=>true).catchError(
          (onError) {
